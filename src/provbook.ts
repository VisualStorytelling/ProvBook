...
import { ICellModel, Cell, CodeCell, MarkdownCell, RawCell } from '@jupyterlab/cells';
...
var getprovdiff = function (
  ...
  var url =
    ...
    "/" +
    path_join(Jupyter.notebook.base_url, "provbookdiff");
...
const provbook = {
  ...
  human_readable_duration: (duration_ms, item_count) => {
    if (duration_ms < 1000) {
    ...
    if (!days) {
      ...
      human_readable_duration +=
        (human_readable_duration ? " " : "") + secs.toFixed(decimals) + "s";
    }
  ...
  get_output: (output) => {
    ...
    for (var index in output) {
      ...
      if (output_type == "execute_result" || output_type == "display_data") {
        ...
        output_entry =
          output_entry + '<div class="para_text_wrap">' + output_val + "</div>";
  ...
  get_provenance_entry: (provenance, execution_index) => {
    ...
    var output_entry = "";
    if (
      "outputs" in provenance[execution_index] &&
      !Array.isArray(provenance[execution_index]["outputs"])
    ) {
      var output_val = provenance[execution_index]["outputs"];
      if (output_val) {
        output_entry =
          '<p class="para_text_wrap">Output:\n' + output_val + "</p>";
        output_entry = get_output(provenance[execution_index]["outputs"]);
      }
    } else if (
      "outputs" in provenance[execution_index] &&
      provenance[execution_index]["outputs"].length > 0
    ) {
      output_entry = get_output(provenance[execution_index]["outputs"]);
    }
    ...
    var prov_hist =
      ...
      execution_time +
      "</p>" +
      '<p class="para_text_wrap">BLAH Source:\n' +
      source +
      "</p>" +
      output_entry;
    return prov_hist;
  ...
  update_provenance_area_textcell: (cell) => {
    ...
    $(".slider-range." + cell_id).slider({
      ...
      create: function (evt, ui) {
        var last_modified =
          cell.metadata["provenance"][min_val]["last_modified"];
        if (last_modified) {
      ...
      slide: function (evt, ui) {
        var last_modified =
          cell.metadata["provenance"][ui.value]["last_modified"];
        if (last_modified) {
          last_modified = moment(last_modified).format();
        }
        $(".slider-time." + cell_id).html(last_modified);
  ...
  // Create slider and provenance area to display the provenance of the text cell.
  create_slider_area_textcell: (cell) => {
    var cell_id = cell.cell_id;
    var slider_area = cell.element.find(".slider_area");
    if (slider_area.length < 1) {
      slider_area = $(
        '<div id="time-range"><p>Modified Time: <span class="slider-time"></span> </p><div class="slider-range"></div></div>'
      )
        .addClass("slider_area")
        .addClass(cell_id)
  ...
  create_slider_area_codecell: (cell) => {
    var cell_id = cell.cell_id;
    var slider_area = cell.element.find(".slider_area");
    if (slider_area.length < 1) {
      slider_area = $(
        '<div id="time-range"><p>Start Time: <span class="slider-time"></span> </p><div class="slider-range"></div></div>'
      )
        .addClass("slider_area")
        .addClass(cell_id)
    ...
    $(".slider-range." + cell_id).slider({
      ...
      create: function (evt, ui) {
        ...
        $(".provenance_area." + cell_id).html(function () {
          ...
          var prov_hist = get_provenance_entry(
            cell.metadata["provenance"],
            min_val
          );
          prov_hist =
            "<p>Number of Runs:" + no_of_executions.length + "</p>" + prov_hist;
          return prov_hist;
        });
      },
      slide: function (evt, ui) {
  ...
  update_provenance_area: (cell) => {
    if (
      !("provenance" in cell.metadata && cell.metadata["provenance"].length > 0)
    ) {
      return;
    }
    if (cell instanceof MarkdownCell || cell instanceof RawCell) {
    ...
    return $(
      Jupyter.toolbar.add_buttons_group([
        Jupyter.keyboard_manager.actions.register(
          ...
          "provenance-selected-cell",
          "provenance_selected_cell_btn"
        ),
        Jupyter.keyboard_manager.actions.register(
          {
        ...
        Jupyter.actions.register(
          ...
          "provenance_diff_selected_cell_btn",
          "provbookdiff"
        ),
      ])
    );
  ...
  update_provenance_metadata: (cell) => {
    Jupyter.notebook.get_cells().forEach(function (cell) {
      if (cell instanceof MarkdownCell || cell instanceof RawCell) {
        update_provenance_metadata_textcell(cell);
      }
  ...
  create_download_rdf_menu: () => {
    ...
    var downloadSubMenu = $(
      '<li id="download_html_embed"><a href="#">RDF (.ttl)</a></li>'
    );
  ...
  create_provenance_book_diff_area: (cell) => {
    ...
    var provenance_book_diff_area = cell.element.find(
      ".provenance_book_diff_area"
    );
    var base_selected_execution = 0,
      remote_selected_execution = 1;
    if (provenance_book_diff_area.length < 1) {
      provenance_book_diff_area = $(
        '<div id="prov_diff_area"><p><strong>Provenance Difference</strong>: Select the options to see the difference between two executions.</p></div>'
      )
        .addClass("provenance_book_diff_area")
        .addClass(cell_id)
        .insertAfter(cell.element.find(".input_area"));
      select_base = $("<select></select>")
        .attr("name", "provenance_diff_base")
      ...
      select_remote = $("<select></select>")
        .attr("name", "provenance_diff_remote")
  ...
  add_option: (select_element, cell_provenance) => {
